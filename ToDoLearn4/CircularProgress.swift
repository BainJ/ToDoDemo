...
public class KDCircularProgress: UIView {
    ...
    private func setInitialValues() { // We have this because didSet effects(效果/作用) are not triggered(触发) when invoked(调用) directly from init method
        radius = (frame.size.width/2.0) * 0.8 //We always apply a 20% padding, stopping glows from being clipped(省略)
        angle = 0
        startAngle = 0
        clockwise = true
...
//        progressLayer.contentsScale = window!.screen.scale
    ...
    private class KDCircularProgressViewLayer: CALayer {
        ...
        var startAngle: Int!  //开始角度
        var clockwise: Bool!   //顺时针
        var roundedCorners: Bool!  //成圆形的角度
        var gradientRotateSpeed: CGFloat!  //梯度循环速度
        var glowAmount: CGFloat!  //生长总数
        var glowMode: KDCircularProgressGlowMode!  //生长模式
        var progressThickness: CGFloat!  //progress宽度
        var trackThickness: CGFloat!  //轨道宽度
        var trackColor: UIColor!  //轨道颜色
        ...
        var gradientCache: CGGradientRef?  //梯度
        var locationsCache: [CGFloat]?
        
        struct GlowConstants {
            static let sizeToGlowRatio: CGFloat = 0.00015  //生长度的大小
        ...
        override func drawInContext(ctx: CGContext!) {
            ...
            //            CGContextAddArc(ctx, CGFloat(size.width/2.0), CGFloat(size.height/2.0), arcRadius, CGFloat(M_PI * 0.3), CGFloat(M_PI * 1.2), 0)
            //            CGContextAddArc(ctx, 100, 100, arcRadius, 0, CGFloat(M_PI * 2), 0)
            trackColor.set()
            ...
            CGContextAddArc(imageCtx, CGFloat(size.width/2.0),CGFloat(size.height/2.0), arcRadius - 10, fromAngle, toAngle, clockwise == true ? 1 : 0)
        ...
        func drawGradientWithContext(ctx: CGContext!, componentsArray: [CGFloat]) {
            ...
            if let g = self.gradientCache {
                ...
                let aa: Int = componentsArray.count / 4
                let g = CGGradientCreateWithColorComponents(baseSpace, componentsArray, locations, UInt(aa))
